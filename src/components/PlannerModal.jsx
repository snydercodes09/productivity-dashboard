...
// ⚡ Bolt: Memoized TimeSlot to prevent unnecessary re-renders of all 24 slots on every keystroke
// Expected Impact: Eliminates input lag when typing out plans by avoiding redundant component re-renders.
const TimeSlot = memo(({ hour, label, value, onInput }) => (
  <div className="flex items-stretch border-b border-white/5 last:border-0 group hover:bg-white/5 transition-colors rounded-lg">
    ...
    <div className="flex-grow">
      <input
        ...
        maxLength={200}
        value={value || ''}
        onChange={(e) => onInput(hour, e.target.value)}
      />
    </div>
  </div>
));
