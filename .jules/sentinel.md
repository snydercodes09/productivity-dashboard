...
## 2026-08-10 - Prevent Local Storage Exhaustion
**Vulnerability:** Client-side Denial of Service (DoS) via local storage exhaustion.
**Learning:** HTML `maxLength` attributes on input elements are insufficient to prevent large data payloads from being saved to local storage, as users can bypass them.
**Prevention:** Always enforce input length limits (e.g., using `.substring()`) at the JavaScript/state level before writing to persistent storage.
